...
linkedList_h* createLinkedList_h(void){
    ...
    L -> head = NULL; // 공백 리스트이므로 NULL로 생성
    return L;
}
...
void insertFirstNode(linkedList_h *L, char *x){
    ...
    strcpy(newNode->data, x); // 새 노드의 데이터 필드에 문자열 x 저장
    newNode -> link = L -> head;
    L -> head = newNode;
}
...
void insertLastNode(linkedList_h *L, char* x){
    ...
    strcpy(newNode->data, x);
    newNode -> link = NULL;
    if (L -> head == NULL){ // 현재 리스트가 공백일 경우
        L -> head = newNode; // 새 노드 리스트의 시작 노드로 연결
        return;
    }
    // 현재 리스트가 공백이 아닐 경우
    temp = L -> head;
...
void deleteNode(linkedList_h *L, listNode* p){
    ...
    if (L -> head == NULL) return; // 공백 리스트라면 연산 중단
    if (L -> head -> link == NULL) { // 리스트에 노드가 한 개만 있는 경우
        free(L->head);
        L->head=NULL; // 첫 번째 노드를 메모리에서 해제하고 NULL로 설정
        return;
    }
...
void reverse(linkedList_h *L){
    ...
    // 리스트의 첫 번째 노드부터 링크를 따라 다음 노드로 이동하면서 노드 간 연결을 바꿈
    while (p != NULL){
        r = q;
        q = p;
        p = p->link;
...
int main()
{
    ...
    printf("(7) Insert Middle Node!\n");
    insertMiddleNode(L, p, "d");
    printList(L); getchar();
